...
/* v01.57: launcher-side sceIo logging channel (vita-port/src/vita_crumb.c).
 * Declared here directly - the MIDP build must not depend on launcher
 * headers. Weak fallback: when the MIDP tree is linked STANDALONE
 * (libmidp.so), no launcher object provides the symbol; the weak no-op
 * keeps that link closed. In the final VPK link the strong definition in
 * vita_crumb.c overrides it, so on-device behavior is unchanged. */
extern void crumb_append(const char *path, const char *s, int length)
    __attribute__((weak));

__attribute__((weak)) void crumb_append(const char *path, const char *s,
                                        int length) {
    (void)path; (void)s; (void)length;
}
...
/* Debug via fprintf to stderr (goes to TTY) - also fflush to force output */
#define DBG(...) do { fprintf(stderr, __VA_ARGS__); fflush(stderr); } while(0)

/* File-based logging to ux0:/data/midp_debug.log for diagnosis in Vita3K.
 * VITA: uses a cached FILE handle (open lazily once, never closed) because
 * repeated fopen/fclose after the VM starts can fail under Vita3K when
 * file handles are exhausted, silently dropping log lines. */
static FILE *_g_midp_dbg = NULL;
static void _midp_dbg_write(const char *fmt, ...) {
    va_list ap;
    if (_g_midp_dbg == NULL) {
        _g_midp_dbg = fopen("ux0:/data/midp_debug.log", "a");
        if (_g_midp_dbg == NULL) return;
    }
    va_start(ap, fmt);
    vfprintf(_g_midp_dbg, fmt, ap);
    va_end(ap);
    fflush(_g_midp_dbg);
}
#define DBGF(...) _midp_dbg_write(__VA_ARGS__)
...
#else
/* Stub for when debugger is disabled */
void JVMSPI_DebuggerNotification(jboolean is_active) {
    (void)is_active;
}
#endif
...
void JVMSPI_PrintRaw(const char* s, int length) {
  /* VITA DEBUG: also write VM output (incl. uncaught exception traces) to a
   * file. v01.28: per-line fopen/fclose (hook style) instead of a cached
   * handle - a permanently-open FILE* keeps its buffer host-side under
   * Vita3K, and a force-kill loses everything that was not fclose()d
   * (the 8-31 empty-log incident). Same scheme as vm_boot.log in
   * JVM_vita.cpp.
   * v01.33: DO NOT append '\n' here. CLDC's PrintStream.write(int) calls
   * this once PER CHARACTER (length == 1), so a forced newline turned the
   * log into one-char-per-line. The strings already carry their own '\n'
   * where the Java code emitted println(); write them through verbatim.
   * v01.57: stdio is OFF LIMITS on this path. PrintStream.write(int)
   * drives this PER CHARACTER, i.e. per-character fopen/fwrite/fclose on
   * a VM worker thread - and the FIRST stdio write from such a thread
   * triggers newlib's lazy FILE-lock init, whose pte_osSemaphoreCreate
   * is exactly where the v01.56 coredump crashed (strge r3,[r5] with
   * r5=0). Route the bytes through the launcher's sceIo-based channel
   * (vita_crumb.c crumb_append) instead: no FILE*, no locks, unbuffered,
   * crash-safe. */
  crumb_append("ux0:/data/vm_output.log", s, length);
...
midp_run_midlet_with_args_cp(SuiteIdType suiteId,
...

    {
        ...
        call_count++;
        DBGF("[midp_run] ENTRY #%d, suiteId=%d, classPathExt=%s, caller=%p\n",
            call_count, (int)suiteId, classPathExt ? classPathExt : "NULL",
            __builtin_return_address(0));
    }

    DBG("===== midp_run_midlet_with_args_cp ENTRY =====\n");
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "  suiteId=%d\n", (int)suiteId);
        DBG(buf);
        if (classPathExt) {
            snprintf(buf, sizeof(buf), "  classPathExt='%s'\n", classPathExt);
            DBG(buf);
        } else {
            DBG("  classPathExt=NULL\n");
        }
    }
    ...
    if (midpInitCallback(VM_LEVEL, midpInitializeVM, midpFinalizeVM) != 0) {
        DBG("  ERR: midpInitCallback failed\n");
        DBGF("[midp_run] midpInitCallback FAILED\n");
        REPORT_WARN(LC_CORE, "Out of memory during init of VM.\n");
        return MIDP_ERROR_STATUS;
    }
    DBGF("[midp_run] midpInitCallback OK\n");
...
#if defined(VITA)
    /* v01.86: disable runtime bytecode verification (root cause of the
     * log9..log12 round-hang family). UseVerifier defaults to true
     * (Globals.hpp), so every lazily-loaded jar class went through
     * Verifier::verify_class on the VM thread; UI*.<clinit> in one game
     * took seconds (quadratic stackmap scan) and froze the whole VM
     * while the ticker stayed alive. MIDlet jars are preverified by the
     * WTK toolchain (J2ME spec), so re-verifying at runtime is redundant
     * here. Must run before midpRunVm - the #if (VERIFY_ONCE) save/
     * restore block below is compiled out in this build. v01.87: use
     * KNI_FALSE - JNI_FALSE is not defined on the midp side and the
     * v01.86 build of this file silently failed, leaving the stale
     * Sep-14 object linked into libobj.a (the verifier stayed on). */
    JVM_SetUseVerifier(KNI_FALSE);
#endif

    /* VITA HACK: limit do-while retries to prevent 21x infinite loop */
    int _vita_iter = 0;

    do {
        _vita_iter++;
        if (_vita_iter > 1) {
            DBGF("[midp_run] breaking do-while after 1 iter\n");
            break;
        }
        ...
        case MIDP_ERROR_OUT_MEM:
            DBG("  ERR: MIDP_ERROR_OUT_MEM\n");
            DBGF("[midp_run] MIDP_ERROR_OUT_MEM\n");
            REPORT_WARN(LC_CORE, "Out of memory: could allocate classpath.\n");
            return MIDP_ERROR_STATUS;

        case MIDP_ERROR_AMS_SUITE_CORRUPTED:
            DBG("  ERR: MIDP_ERROR_AMS_SUITE_CORRUPTED\n");
            DBGF("[midp_run] MIDP_ERROR_AMS_SUITE_CORRUPTED\n");
            REPORT_WARN(LC_CORE, "I/O error reading appdb.\n");
            return MIDP_ERROR_STATUS;

        case MIDP_ERROR_AMS_SUITE_NOT_FOUND:
            DBG("  ERR: MIDP_ERROR_AMS_SUITE_NOT_FOUND\n");
            DBGF("[midp_run] MIDP_ERROR_AMS_SUITE_NOT_FOUND\n");
            REPORT_WARN(LC_CORE, "Suite not found.\n");
            return SUITE_NOT_FOUND_STATUS;

        default:
            DBG("  OK: classPath built\n");
            DBGF("[midp_run] classPath built OK, status=%d\n", (int)status);
            break;
        }
         ...
         */
        DBG("  Calling midpRunVm\n");
        DBGF("[midp_run] before midpRunVm, classPath set\n");
        vmStatus = midpRunVm(classPath, MIDP_MAIN, 0, NULL);
        {
            char buf[128];
            snprintf(buf, sizeof(buf), "  midpRunVm returned vmStatus=%d (MAIN_EXIT=%d)\n", vmStatus, MAIN_EXIT);
            DBG(buf);
            DBGF("[midp_run] after midpRunVm, vmStatus=%d (MAIN_EXIT=%d)\n", vmStatus, MAIN_EXIT);
        }
    ...
    } while (commandState->suiteId != UNUSED_SUITE_ID && commandState->vmRestarted == KNI_FALSE);
...
static MIDP_ERROR getClassPathPlus(SuiteIdType suiteId,
    ...
    DBG("[getClassPathPlus] start\n");
    DBGF("[getClassPathPlus] start suiteId=%d classPathExt='%s'\n",
        (int)suiteId, classPathExt ? classPathExt : "NULL");
    {
        char buf[256];
        snprintf(buf, sizeof(buf), "  suiteId=%d classPathExt='%s'\n",
            (int)suiteId, classPathExt ? classPathExt : "NULL");
        DBG(buf);
    }

    if (suiteId == UNUSED_SUITE_ID) {
        DBG("  ERR: UNUSED_SUITE_ID -> NOT_FOUND\n");
        DBGF("[getClassPathPlus] UNUSED_SUITE_ID -> NOT_FOUND\n");
        return MIDP_ERROR_AMS_SUITE_NOT_FOUND;
    }

    if (suiteId == INTERNAL_SUITE_ID) {
        DBG("  INFO: INTERNAL_SUITE_ID, jarPath=EMPTY\n");
        DBGF("[getClassPathPlus] INTERNAL_SUITE_ID branch\n");
        ...
        if (error != ALL_OK) {
            DBGF("[getClassPathPlus] installed suite err=%d -> NOT_FOUND\n", (int)error);
            return MIDP_ERROR_AMS_SUITE_NOT_FOUND;
        }
    }
    ...
    {
        char buf[128];
        int allocSize = sizeof(JvmPathChar) * (jarPathLen + 1 + additionalPathLength + 1);
        snprintf(buf, sizeof(buf), "  getClassPathPlus: jarPathLen=%d allocSize=%d additionalPathLength=%d\n",
            (int)jarPathLen, allocSize, additionalPathLength);
        DBG(buf);
    }
    
    /* For INTERNAL_SUITE_ID with empty jarPath, parse additionalPath for multiple jars */
    if (jarPathLen == 0 && additionalPathLength > 0) {
        DBG("  INFO: INTERNAL_SUITE_ID, jarPath=EMPTY, parsing additionalPath\n");
        
        /* Parse additionalPath which may contain multiple jars separated by ':' */
        int pathCount = 1;
        int k;
        for (k = 0; k < additionalPathLength; k++) {
            if (additionalPath[k] == ':') {
                pathCount++;
            }
        }
        
        DBG("  additionalPath contains multiple paths, will parse\n");
        
        /* Build classpath with proper separators */
        newPath = (JvmPathChar*)midpMalloc(sizeof(JvmPathChar) * (additionalPathLength + pathCount + 1));
        if (NULL == newPath) {
            DBG("  ERR: midpMalloc failed for newPath\n");
            pcsl_string_free(&jarPath);
            return MIDP_ERROR_OUT_MEM;
        }
        
        /* Copy and replace ':' with path separator */
        int destIdx = 0;
        for (j = 0; j < additionalPathLength; j++, destIdx++) {
            if (additionalPath[j] == ':') {
                newPath[destIdx] = (JvmPathChar)getCharPathSeparator();
            } else {
                newPath[destIdx] = (JvmPathChar)additionalPath[j];
            }
        }
        newPath[destIdx] = 0;
        ...
        {
            char cp_dbg[512];
            int plen = destIdx < 511 ? destIdx : 511;
            for (k = 0; k < plen; k++) cp_dbg[k] = (char)(newPath[k] & 0xFF);
            cp_dbg[plen] = 0;
            {
                char cp_msg[600];
                snprintf(cp_msg, sizeof(cp_msg), "  [getClassPathPlus] parsed classPath='%s'\n", cp_dbg);
                fprintf(stderr, "%s", cp_msg);
                fflush(stderr);
            }
        }
        
        *userClassPath = newPath;
        DBG("  [getClassPathPlus] OK returning (parsed multi-path)\n");
    ...
    if (NULL == jarPathData) {
        DBG("  ERR: pcsl_string_get_utf16_data returned NULL\n");
        pcsl_string_free(&jarPath);
        return MIDP_ERROR_OUT_MEM;
    }
    ...
    *userClassPath = newPath;

    {
        ...
        char buf[1100];
        int k;
        int plen = (jarPathLen + (jarPathLen > 0 ? 1 : 0) + additionalPathLength);
        if (plen > 1023) plen = 1023;
        for (k = 0; k < plen; k++) cp_dbg[k] = (char)(newPath[k] & 0xFF);
        cp_dbg[plen] = 0;
        snprintf(buf, sizeof(buf), "  result classPath='%s' (len=%d)\n", cp_dbg, plen);
        DBG(buf);
    }

    setMonetClassPath(userClassPath, jarPathLen);

    DBG("  [getClassPathPlus] OK returning\n");
    return MIDP_ERROR_NONE;
}
