...
public:
  ...
  inline static bool system_text_contains(const OopDesc* target) {
    ...
    // Portability fix: on 64-bit cross-generator hosts, use 64-bit
    // pointer math to avoid truncation when target is far from the
    // text segment.
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    const address_word offset = (address_word)target - (address_word)_min_text_seg_addr;
    if (offset < (address_word)_text_total_size) {
      return true;
    }
#else
...
#else
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    address_word offset = (address_word)target - (address_word)&_rom_text_block[0];
    if (offset < (address_word)_rom_text_block_size_fast) {
      return true;
    }
#else
...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    address_word offset = (address_word)target - (address_word)&_rom_data_block[0];
    if (offset < (address_word)_rom_data_block_size_fast) {
      return true;
    } else {
      return false;
    }
...
#if !defined(PRODUCT) || USE_PRODUCT_BINARY_IMAGE_GENERATOR \
       || ENABLE_JVMPI_PROFILE || USE_SOURCE_IMAGE_GENERATOR
  /*
