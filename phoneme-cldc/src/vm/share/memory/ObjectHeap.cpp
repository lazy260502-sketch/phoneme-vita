...
// MIDP native code uses SNI_BEGIN_RAW_POINTERS/SNI_END_RAW_POINTERS macros
// which reference _jvm_in_raw_pointers_block. In PRODUCT mode these macros
// are empty, but MIDP is compiled WITHOUT PRODUCT defined, so the symbol
...
size_t ObjectHeap::update_slices_size(size_t object_heap_size) {
  ...
  // to heap start.  On 64-bit cross-generator hosts, near_offset is in
  // OopDesc** units (8 bytes each), so we must shift by LogBytesPerOop.
  size_t near_bits = 0;
  size_t s;
  for (s = (object_heap_size - 1) >> LogBytesPerOop; s != 0; s >>= 1) {
    near_bits++;
  }
  ...
  size_t slice_size_in_bytes = _slice_size * BytesPerOop;
  _nof_slices  =
...
bool ObjectHeap::adjust_heap_size(size_t target_heap_size) {
  ...
  // bitvector_size: number of bytes for the GC bitvector.  Each bit covers
  // BytesPerOop bytes of heap, and each juint (4 bytes) holds BitsPerWord=32
  // bits, so: bitvector_size = target_heap_size / (BitsPerWord * BytesPerOop)
  //                            * sizeof(juint)
  //                         = target_heap_size * sizeof(juint)
  //                            / (BitsPerWord * BytesPerOop)
  // On 32-bit (BytesPerOop==4): target_heap_size / 32  (same as before)
  // On 64-bit (BytesPerOop==8): target_heap_size / 64
  const size_t bitvector_size =
      (target_heap_size * sizeof(juint)) / (BitsPerWord * BytesPerOop);
  ...
  if (is_new_heap) {
    ...
    if (_heap_min == _heap_capacity) {
      ...
      max_bitv = (_heap_capacity * sizeof(juint)) / (BitsPerWord * BytesPerOop) + max_slices_size;
  ...
  // Portability fix: _heap_size is in bytes, so use byte-based DERIVED()
  // instead of OopDesc** arithmetic (which is off by 2x on a 64-bit host).
  OopDesc** const heap_top = DERIVED(OopDesc**, _heap_start, _heap_size);
  _heap_top         = heap_top;
  ...
  _bitvector_base =
      _bitvector_start - (((uintptr_t)_heap_start >> LogBytesPerOop) / BitsPerByte);
...
inline OopDesc* ObjectHeap::rom_oop_from_offset(size_t offset,
                                                const QuickVars& qv) {
  if ((offset & 0x1) == 0) {
    return (OopDesc*)(int(address_word(qv.rom_text_start)) + offset);
  } else {
    offset --;
    return (OopDesc*)(int(address_word(qv.rom_data_start)) + offset);
  }
}
...
inline OopDesc* ObjectHeap::rom_oop_from_offset(size_t offset) {
  if ((offset & 0x1) == 0) {
    ...
    return (OopDesc*)(int(address_word(ROM::min_text_seg_addr())) + offset);
#else
    return (OopDesc*)(int(address_word(&_rom_text_block[0])) + offset);
#endif
  } else {
    offset --;
    return (OopDesc*)(int(address_word(&_rom_data_block[0])) + offset);
  }
}
...
#ifdef VITA_GC_DEBUG
      // VITA: per-object GC compaction trace. Ungated this floods stderr with
      // one line per live object per GC (the 0905 music-toggle 'hang' - same
      // failure mode as the ClassFileParser dumps fixed by 2959a08).
      fprintf(stderr, "GC encode: obj=%p near=%p near_off=0x%lx slice_off=%lu encoded=%p\n",
              (void*)obj, (void*)obj_near, (unsigned long)near_offset,
              (unsigned long)slice_offset, (void*)obj->_klass);
#endif
  ...
  while (this_stack != NULL) {
    ...
    if ((OopDesc**)this_stack < _end_fixed_objects) {
      ...
      *previous_stack_addr = next_stack;
      ...
      int delta = (int)(address_word)destination - (int)(address_word)this_stack;
  ...
  WRITE_BARRIER_OOPS_LOOP_BEGIN(start, end, p);
  {
    ...
    if (instance_size > 0) {
      ...
      if (instance_size == InstanceSize::size_compiled_method) {
        int delta = (int)(address_word)destination - (int)(address_word)p;
...
#if defined(VITA)
  vita_site_mark(); /* v01.84: log10 - GC entry marker */
#endif
...
void ObjectHeap::verify_layout() {
  ...
  if (_heap_top > _heap_limit) {
    fprintf(stderr, "DBG verify_layout: ret=%p heap_start=%p heap_top=%p heap_limit=%p heap_end=%p loa_bottom=%p ctop=%p inline_end=%p cstart=%p\n",
            __builtin_return_address(0),
            (void*)_heap_start, (void*)_heap_top, (void*)_heap_limit,
            (void*)_heap_end, (void*)_large_object_area_bottom,
            (void*)_compiler_area_top, (void*)_inline_allocation_end,
            (void*)_compiler_area_start);
  }
...
inline void* ObjectHeap::set_heap_limit( void* new_heap_limit ) {
  ...
  if( !in_use && _heap_min == _heap_capacity ) {
    ...
    new_heap_limit = (void*)align_down(unsigned(address_word(new_heap_limit)));
#ifdef AZZERT
    ...
    if( reduction ) {
      ...
      _heap_top = heap_top;
      _heap_limit = heap_top + MimimumMarkingStackSize;
      fprintf(stderr, "DBG set_heap_limit: new_limit=%p reduction=%d old_top=%p new_top=%p new_heap_limit_after=%p\n",
              new_heap_limit, reduction, (void*)old_heap_top, (void*)heap_top,
              (void*)_heap_limit);
...
void ObjectHeap::print(Stream* st) {
  st->print_cr("Object heap       [0x%x,0x%x), %d bytes", 
                                   ...
                                   int(address_word(_heap_top)) - int(address_word(_heap_start)));
  ...
  st->print_cr("Bit vector          [0x%x,0x%x), %d bytes",
                                  ...
                                  int(address_word(_slices_start)) - int(address_word(_bitvector_start)));
