...
private:  
 ...
 private:
  ...
  static address_word oop_index (OopDesc** p) {
    GUARANTEE(contains(p), "Should be in object heap");
    // Portability fix: use full address_word (uintptr_t) for the index, so
    // that high 64-bit host pointers are not truncated before shifting.
    // Use LogBytesPerOop (sizeof(OopDesc*) log2) so the bitvector stays
    // aligned with OopDesc** pointer arithmetic on 64-bit hosts.
    return address_word(p) >> LogBytesPerOop;
  }

  static address_word bitvector_word_index  (const address_word i) {
    return i >> LogBitsPerWord;
  }

  static unsigned bitvector_bit_index   (const address_word i) {
    return (unsigned)(i & (BitsPerWord-1));
  }

  static unsigned bitvector_bit_mask    (const address_word i) {
    return 1 << bitvector_bit_index(i);
  }

  static unsigned& bitvector_word( const address_word i,
                                   address bitvector_base = _bitvector_base) {
...
public:
  static juint* get_bitvectorword_for_aligned(OopDesc** p) {
    ...
    return DERIVED(juint*, _bitvector_base,
                     ((uintptr_t)p) >> (LogBytesPerOop+LogBitsPerByte));
  }
private:
  ...
  static OopDesc** get_aligned_for_bitvectorword(juint *ptr) {
    return (OopDesc**)
        (DISTANCE(_bitvector_base, ptr) << (LogBytesPerOop + LogBitsPerByte));
  }
  ...
  enum {
    alignment = BitsPerWord * BytesPerOop
  };
 ...
 */
inline unsigned int* word_for(address_word offset, unsigned int* map) { 
  return map + (offset/BitsPerWord); 
}

inline void bit_clear(address_word offset, unsigned int* map) {
  intptr_t* w = (intptr_t*)word_for(offset, map);
  int pos = (int)(offset%BitsPerWord);
  clear_nth_bit(*w, pos);
}

inline void bit_set(address_word offset, unsigned int* map) {
  intptr_t* w = (intptr_t*)word_for(offset, map);
  int pos = (int)(offset%BitsPerWord);
  set_nth_bit(*w, pos);
}

inline bool is_bit_set(address_word offset, unsigned int* map) {
  return is_set_nth_bit(*word_for(offset, map), (int)(offset%BitsPerWord)); 
}

inline void ObjectHeap::set_bit_for(OopDesc** p, address bitvector_base) {
  const address_word i = oop_index(p);
...
inline void ObjectHeap::set_bit_range(OopDesc** start, int len) {
  address_word t = oop_index(start);
