...
void midp_file_cache_close(char** ppszError, int handle) {
    ...
    *ppszError = NULL;
    ...
    if (mFileCache != NULL && mFileCache->handle == handle) {
        ...
        /* VITA FIX (v01.48): drop the singleton after flushing it. The
         * cache kept the closed file's handle value forever; that
         * handle is freed by pcsl_file_close, and a later open of
         * another file can malloc back the SAME VitaFileHandle memory
         * with a live fd. The next midp_file_cache_open then took the
         * "second file" else-branch and ran finalize(stayOpen=TRUE)
         * against the stale handle value -> storagePosition() seeked
         * the NEW file to the OLD file's position, so the freshly
         * opened record store read its db header from a garbage
         * offset ("invalid record store contents") and the MIDlet
         * saw its saved data as corrupt (settings reset every launch,
         * bookmark save wedged). Freeing the singleton here restores
         * the upstream invariant that mFileCache->handle always
         * designates an OPEN file. */
        midpFree(mFileCache);
        mFileCache = NULL;
    }
