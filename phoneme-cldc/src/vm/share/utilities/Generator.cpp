...
void Generator::generate_oopmap(Stream* stream, 
  ...
  for (i = 0; i < count; i++) { 
    ...
    // Oop map entries are in units of OopDesc* slots (sizeof(OopDesc*)),
    // because map_oops_do() uses OopDesc** pointer arithmetic (base += entry).
    // On 32-bit, sizeof(OopDesc*)==BytesPerWord==4, so this is unchanged.
    // On 64-bit cross-generator, sizeof(OopDesc*)==8, so we divide by 8.
    delta = delta / (int)sizeof(OopDesc*);
...
#if !defined(PRODUCT) || USE_SOURCE_IMAGE_GENERATOR
  ...
  for (;;) {
    ...
    if (jvm_strcmp(name, stored_name) == 0) {
      ...
      if (stored_offset != offset) {
        tty->print_cr("OOPMAP MISMATCH: class=%s field=%s loopgen_offset=%d runtime_offset=%d delta=%d",
                      _checking_oopmap_class, name, (int)stored_offset, (int)offset, (int)(offset - stored_offset));
      }
