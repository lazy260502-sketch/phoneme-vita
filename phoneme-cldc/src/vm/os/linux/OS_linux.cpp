...
// Fix for modern Linux/GCC: SA_RESTART, SA_SIGINFO and siginfo fields
// may be hidden behind _GNU_SOURCE or feature test macros.
// _GNU_SOURCE must be defined BEFORE any system header is included.
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
#if defined(__x86_64__) && defined(__linux__) && !defined(__NetBSD__)
#include <execinfo.h>
#include <unistd.h>
#endif
...
static void print_siginfo(siginfo_t *si) {
  ...
  jvm_fprintf(stderr,
	      ...
	      name, si->si_errno, si->si_code, (void*)si->si_addr);
#else
  jvm_fprintf(stderr,
	      "Fatal signal %s: code=%d\n",
	      name, si->si_code);
#endif
}
...
static void print_ucontext(void* context) {
  ...
  jvm_fprintf(stderr, "RIP=%p RSP=%p RBP=%p\n",
...
static void handle_segv_siginfo(int signo, siginfo_t *info, void *context) {
...
#if defined(__x86_64__) && defined(__linux__) && !defined(__NetBSD__)
  {
    ...
    jvm_fprintf(stderr, "Backtrace (%d frames):\n", n);
...
void Os::initialize() {
  ...
  int sa_flags = 0; // SA_RESTART not available in VitaSDK's newlib
