...
#if ENABLE_ROM_GENERATOR && (!defined(PRODUCT) || USE_SOURCE_IMAGE_GENERATOR)
...
// Force ENABLE_TTY_TRACE=1 so all #if !defined(PRODUCT) || ENABLE_TTY_TRACE
// guarded code is visible in PRODUCT mode when building ROM generator.
#undef  ENABLE_TTY_TRACE
