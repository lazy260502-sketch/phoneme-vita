...
ReturnOop BinaryAssembler::find_literal(const Oop* oop, int imm32, 
...

  {
    ...
    {
      // access_literal_pool() encodes the PC-relative offset as
      //     _bci - (_code_offset + 8)
      // which must fit into 12 bits, so the furthest reusable literal is
      //     _code_offset - (2^12 - 1) + 8  ==  _code_offset - 4087.
      // "offset" above is 4088, i.e. one byte too permissive: a literal
      // written exactly "offset" bytes behind the current instruction used
      // to be accepted here and then turned into an immediate of -4096,
      // which trips Assembler::check_imm() in a debug build and is silently
      // truncated to 0 (wrong address) in a product build.  The non-VFP
      // variant below rejects that case, so keep the window one byte tighter.
      const int min_offset = _code_offset - offset + 1;
