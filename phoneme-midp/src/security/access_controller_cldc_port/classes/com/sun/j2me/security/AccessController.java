...
public final class AccessController {
    ...
    public static void checkPermission(String name,
        ...
        if (context == null) {
            /* VITA: bootstrap window. The suite loader reads the jar
             * MANIFEST (JarReader.readJarEntry -> checkPermission)
             * during createMIDletSuite(), BEFORE initSuiteEnvironment()
             * installs the per-suite context. Only trusted loader code
             * runs in this window (MIDlet classes cannot execute before
             * their suite context exists), so permit instead of
             * throwing - upstream throws because internal suites never
             * carried jar properties; ours do (v01.22 arg0=.jar). */
            return;
        }
