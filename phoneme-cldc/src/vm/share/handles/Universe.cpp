...
bool Universe::bootstrap_with_rom(const JvmPathChar* classpath) {
  ...
  *(OopDesc**)scheduler_waiting() =
  ...
  // On Vita, the hidden flag is not set during ROM generation
  // because ENABLE_ROM_GENERATOR=0. Skip this check on Vita
  // since there's no real security risk in this sandboxed environment.
  // (Per-class stderr warnings removed: hundreds of ROM classes would
  //  each print one line and flood the log - see ClassFileParser.cpp.)
...
bool Universe::bootstrap_without_rom(const JvmPathChar* classpath) {
  ...
  *global_refs_array() = RefArray::initialize(JVM_SINGLE_ARG_CHECK_0);
...
ReturnOop Universe::new_instance(InstanceClass* klass JVM_TRAPS) {
  ...
  // On 64-bit cross-generator hosts, allocate host size (not target size)
  size_t alloc_size = align_allocation_size(sizeof(OopDesc) +
                            (size_t)(instance_size.fixed_value() - TargetOopDescSize) * 2);
#else
  size_t alloc_size = instance_size.fixed_value();
#endif
  OopDesc* result = ObjectHeap::allocate(alloc_size JVM_ZCHECK(result));
...
ReturnOop Universe::new_symbol(TypeArray *byte_array, utf8 name, int length
  ...
  if( result ) {
    OopDesc* proto_near = symbol_class()->prototypical_near();
    result->initialize(proto_near, length);
    if (byte_array) {
