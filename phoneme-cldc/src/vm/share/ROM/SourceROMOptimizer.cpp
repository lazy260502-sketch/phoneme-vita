      ...
      for (int i = 0; i < fields().length(); i += 5) {
        ...
        if (f.is_static()) {
          ...
          if ((dst != DEAD_FIELD) && (type == T_OBJECT || type == T_ARRAY)) {
            ...
            // because new_offset is computed using BytesPerWord, and the oop
            // map is part of the ROM image used on the target where
            // oopSize == BytesPerWord == 4.
            int word_delta = (new_offset - prev_oop_offset) / BytesPerWord;
