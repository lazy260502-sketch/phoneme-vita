...
// Portability fix: sun.misc.Compare/Sort removed in modern JDKs.
// Replaced with a local quicksort implementation below.

// Portability fix: sun.misc.Compare removed in modern JDKs.
// We implement our own compare method directly in the class.
public
class ConstantPool {
	...
	// Sorting the ConstantObject with descending reference
        // count. Portability fix: replaced sun.misc.Sort.quicksort
        // (removed in modern JDKs) with Collections.sort.
        java.util.Vector v = new java.util.Vector();
        for (int i = 0; i < arr.length; i++) {
            v.addElement(arr[i]);
        }
        java.util.Collections.sort(v, new java.util.Comparator() {
            public int compare(Object o1, Object o2) {
                ConstantObject c1 = (ConstantObject)o1;
                ConstantObject c2 = (ConstantObject)o2;
                return c2.nSlots - c1.nSlots;  // descending
            }
        });
        for (int i = 0; i < arr.length; i++) {
            arr[i] = (ConstantObject)v.elementAt(i);
        }
	t.removeAllElements();
	t.addElement(null);
	for (int i = 0; i < arr.length; i++) {
