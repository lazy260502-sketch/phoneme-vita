...
#endif
  // Portability fix: bool is #defined as jint; compare pointer explicitly
  ReturnOop result = Compiler::resume_compilation(this JVM_MUST_SUCCEED);
...
bool Method::compile(int active_bci, bool resume JVM_TRAPS) {
  ...
  // Portability fix: bool is #defined as jint; compare pointer explicitly
  ReturnOop result = Compiler::compile(this, active_bci JVM_MUST_SUCCEED);
  bool status = (result != NULL);
  return status;
}
#endif
...
void Method::update_rom_default_entries() {
  ...
  MethodVariablePart *end =
    (MethodVariablePart *) (((int)(address_word)ptr) + _rom_method_variable_parts_size);
...
#if !defined(PRODUCT) || ENABLE_TTY_TRACE || USE_DEBUG_PRINTING || USE_OOP_VISITOR || USE_SOURCE_IMAGE_GENERATOR
