...
size_t InstanceClass::last_nonstatic_oop_offset() const {
  ...
  while ((value = oop_map_at(map_offset++)) != OopMapSentinel) {
    ...
    // because oop map values are in target word units (4 bytes).
    oop_offset += value*BytesPerWord;
  }
  return oop_offset;
}
...
#if !defined(PRODUCT) || USE_OOP_VISITOR || USE_SOURCE_IMAGE_GENERATOR
...
#endif //!PRODUCT || USE_OOP_VISITOR || USE_SOURCE_IMAGE_GENERATOR
