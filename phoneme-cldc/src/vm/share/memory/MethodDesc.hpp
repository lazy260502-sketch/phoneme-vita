...
class MethodDesc: public OopDesc { 
 private:
  ...
  static size_t allocation_size(int byte_code_size) {
    size_t size = header_size() + byte_code_size * sizeof(jbyte);
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    // On 64-bit cross-generator, address (native code pointer) is 8 bytes,
    // but native_code_offset() aligns to BytesPerWord=4. Ensure enough space
    // for the native code pointer to prevent overflow into adjacent objects.
    int nco = align_size_up(header_size() + 1, BytesPerWord);
    size_t min_size = (size_t)nco + sizeof(address);
    if (size < min_size) {
      size = min_size;
    }
#endif
