...
# include <ctype.h>
# include <string.h>
/* Portability fix: include unconditionally (Vita target is not LINUX but
   still needs uintptr_t for address_word). */
# include <stdint.h>
# include <stdarg.h>
# include <stdlib.h>
# include <stddef.h>
...
// NOTE: Original 0xe6000010 is undefined instruction - causes crash on Vita!
// Use 0xEF000001 (SWI 1) which is properly handled by Vita kernel
#  define BREAKPOINT __asm__ (".inst 0xEF000001")
#  define panic()  __asm__(".word 0xEF000001")
inline unsigned int _rotr(unsigned int x, unsigned int y) {
    int result;
...
// Portability fix: avoid signed overflow warning in modern GCC.
// max_jlong is all bits set except the sign bit.
