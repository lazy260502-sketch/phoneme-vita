...
#if defined(VITA)
  vita_site_mark(); /* v01.84: log10 - death-path branch marker */
#endif
...
#if defined(VITA)
    vita_site_mark(); /* v01.84: log10 - ring must see task cleanup */
#endif
...
#if defined(VITA)
  vita_site_mark(); /* v01.84: log10 - ring must see thread death */
#endif
  SETUP_ERROR_CHECKER_ARG;
  Thread *thread = Thread::current();
  if (TraceThreadsExcessive) {
...
void Thread::finish() {
#if defined(VITA)
  vita_site_mark(); /* v01.84: log10 - ring must see thread finish */
#endif
  SETUP_ERROR_CHECKER_ARG;
  Thread *thread = Thread::current();
