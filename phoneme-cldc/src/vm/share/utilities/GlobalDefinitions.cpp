...
void check_basic_types() {
  // jobject is an opaque pointer type. On 32-bit targets/build hosts it is
  // 4 bytes; on 64-bit build hosts (loopgen/romgen run on x86_64) it is 8
  // bytes. The real invariant is that it is the size of a pointer.
  GUARANTEE( sizeof(void*) == sizeof( jobject), "wrong size for basic type");
