...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    // Use 64-bit pointer math on 64-bit hosts to avoid truncation
    address_word diff = (address_word)obj - (address_word)_rom_text_block_segments[pass];
    if (diff < (address_word)_rom_text_block_segment_sizes[pass]) {
      return pass;
    }
#else
    offset = ((int)(address_word)obj) - ((int)_rom_text_block_segments[pass]);
...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
  address_word p = (address_word)target - (address_word)&_rom_heap_block[0];
  bool ret = p < (address_word)_rom_heap_block_size;
#else
...
#endif
#if !ROMIZED_PRODUCT && !USE_SOURCE_IMAGE_GENERATOR && !CROSS_GENERATOR
...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
  juint byte_offset = (juint)((address_word)obj - (address_word)_rom_text_block_segments[pass]);
#else
  juint byte_offset = (juint)(address_word)obj - _rom_text_block_segments[pass];  
#endif
#else
  juint byte_offset = ((juint)(address_word)obj) - ((juint)(address_word)&_rom_text_block[0]);
