...
// Portability: on a 64-bit cross-generator host, oop fields are 8 bytes
// whereas jint is only 4.  Use sizeof(OopDesc*) for streaming stride.
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
#define CP_OOP_SIZE ((int)sizeof(OopDesc*))
#else
#define CP_OOP_SIZE ((int)sizeof(jint))
#endif
...
void ROMWriter::start(JVM_SINGLE_ARG_TRAPS) {
  ...
  _method_start_skip = TargetOopDescSize/BytesPerWord + 4;
...
void ROMWriter::stream_method(Method* method JVM_TRAPS) {
  // These GUARANTEEs and put_oop_field offsets access HOST memory,
  // so must use sizeof(OopDesc) (host value), not TargetOopDescSize.
...
int ROMWriter::generate_instance_java_fieldmap(InstanceClass* klass) {
  int map_index = 0;
  size_t instance_size = TargetOopDescSize; // the first X bytes are for the object header
...
void ROMWriter::generate_fieldmap_by_oops_do(OopDesc**p) {
  int offset = (int)(address_word)p - (int)(address_word)_streaming_oop;
...
int ROMWriter::gen_and_stream_fieldmap(Oop* object, int force_skip_words
  ...
  int skip_bytes_host = 0;
  int start_offset = get_next_field_map_item_index(0, skip_bytes, field_count,
                                                   &skip_bytes_host);

  // Stream the fields
  return stream_fields_by_map(object, skip_bytes_host, start_offset, 
...
int ROMWriter::get_next_field_map_item_index(int index,
  ...
  if (skip_bytes <= 0) {
    if (host_bytes != NULL) *host_bytes = 0;
    return index;
  }

  int bytes_touched = 0;
  int host_bytes_touched = 0;
  ...
  while (bytes_touched < skip_bytes && index < field_count) {
    switch (current_fieldmap()->byte_at(index++)) {
      ...
      case T_SHORT:
        bytes_touched += 2;
        host_bytes_touched += 2;
        break;
      case T_LONG:
      case T_DOUBLE:
        bytes_touched += 8;
        host_bytes_touched += 8;
        break;
      case T_INT:
      case T_FLOAT:
        bytes_touched += 4;
        host_bytes_touched += 4;
        break;
      case T_OBJECT:
      case T_ARRAY:
      case T_SYMBOLIC:
        bytes_touched += 4;
        host_bytes_touched += (int)sizeof(jobject);
        break;
      default:
        SHOULD_NOT_REACH_HERE();
    }
  ...
  if (host_bytes != NULL) *host_bytes = host_bytes_touched;
  return index;
}
...
void BlockTypeFinder::find_array_type(Oop *owner, Oop *object JVM_TRAPS) {
  ...
  if (object->is_byte_array() && owner != NULL && owner->is_constant_pool()) {
    ...
    my_skip_words = TargetOopDescSize/BytesPerWord + 1; // _klass and _length
    int i;
    for (i=0; i<len; i++) {
  ...
  if (my_skip_words == -1) {
    ...
    if (type == ROMWriter::TEXT_BLOCK && 
      ...
      my_skip_words = TargetOopDescSize/BytesPerWord;
    } else {
      my_skip_words = 0;
    }
  }
...
void BlockTypeFinder::find_type(Oop *owner, Oop *object JVM_TRAPS) {
  ...
  // Check if this is a one-word (header-only) text object.
  // On 64-bit cross-generator hosts, object_size() returns the host size
  // (which may be overestimated), so check the target instance_size instead.
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
  if (FarClass(object->blueprint()).instance_size_as_jint() == (int)align_allocation_size(TargetOopDescSize)
      && my_type == ROMWriter::TEXT_BLOCK) {
#else
  if (object->object_size() == (int)sizeof(OopDesc)
      && my_type == ROMWriter::TEXT_BLOCK) {
#endif
                                ...
                                int &my_pass, int &my_skip_words)
{
  ...
  if (!ROMWriter::write_by_reference(method) && 
    ...
    GUARANTEE(Method::constants_offset() ==
              (int)((TargetOopDescSize/BytesPerWord) * objsize), "Sanity");
    GUARANTEE(Method::exception_table_offset() ==
              (int)((TargetOopDescSize/BytesPerWord + 1) * objsize), "Sanity");
    GUARANTEE(Method::stackmaps_offset() ==
              (int)((TargetOopDescSize/BytesPerWord + 2) * objsize), "Sanity");
    GUARANTEE(Method::heap_execution_entry_offset()==
              (int)((TargetOopDescSize/BytesPerWord + 3) * objsize), "Sanity");
    
    my_skip_words = TargetOopDescSize/BytesPerWord + 1; // skip near, constants
