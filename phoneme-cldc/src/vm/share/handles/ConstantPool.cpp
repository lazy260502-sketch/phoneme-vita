...
int ConstantPool::generate_fieldmap(TypeArray* field_map) {
  ...
  // On 64-bit hosts, each CP slot is 8 bytes.  Worst case: every entry
  // is an int (T_INT + T_INT padding = 2 fieldmap entries per slot).
  // Header: T_OBJECT + T_SHORT + T_SHORT + T_INT(pad) + T_OBJECT = 5.
  int max_length = length() * 2 + 8;
#else
  ...
  field_map->byte_at_put(map_index++, T_SHORT);
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
  // On 64-bit hosts, _tags (8-byte pointer) is 8-byte aligned, so there
  // are 4 bytes of padding after _pad.  Emit a T_INT placeholder.
  ...
  // On 64-bit cross-generator hosts, sizeof(OopDesc) is 8 and
  // sizeof(ConstantPoolDesc) includes an 8-byte _tags pointer, so the
  // original equality no longer holds.  Skip the check there.
#if !(CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__)))
  ...
  for (int i = 0; i < length(); ) {
    if (tag_at(i).is_oop()) {
      ...
      field_map->byte_at_put(map_index++, T_INT);
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
      // 8-byte slot, but T_INT only advances 4 bytes — add padding.
      field_map->byte_at_put(map_index++, T_INT);
#endif
      i++;
    }
...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
      // 2 slots = 16 bytes, but T_DOUBLE only advances 8 bytes — add padding.
      field_map->byte_at_put(map_index++, T_INT);
      field_map->byte_at_put(map_index++, T_INT);
#endif
      i+= 2;
...
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
      // 2 slots = 16 bytes, but T_LONG only advances 8 bytes — add padding.
      field_map->byte_at_put(map_index++, T_INT);
      field_map->byte_at_put(map_index++, T_INT);
#endif
      i+= 2;
