...
/* ========================================================================
 * Vita cross-build: weak stubs for MIDP-side native methods.
 * The real implementations live in MIDP (src/vita_stubs.c).
 * For CLDC's standalone cldc_vm_g executable, provide trivial fallbacks
 * so the ARM target links without unresolved symbols.
 * ======================================================================== */
extern "C" {
  void Java_com_sun_cldchi_jvm_JVM_flushJarCaches(void) {
    /* no-op in standalone CLDC VM */
  }

  jlong Java_com_sun_cldchi_jvm_JVM_monotonicTimeMillis(void) {
    /* For standalone CLDC VM, return a monotonic-ish value */
    return (jlong)0;
  }
}
