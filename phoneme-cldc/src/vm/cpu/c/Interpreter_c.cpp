...
#define WIDE_OFFSET       255
  ...
  /* VITA: legal handler window [min,max], sampled from the initialized
   * table itself. All bc_impl_* handlers live in .text and ROM text blocks
   * live in .rodata ABOVE .text, so a handler clobbered with ROM bytecode
   * data (v01.06: r2=0x8112bbf0 passed the old fixed [0x81000000,0x81400000)
   * window because ROM text sits inside it) is rejected by this tighter
   * window before blx jumps into bytecode data. See dispatch check in
   * Interpret(). */
  static func_t interpreter_handler_min;
  static func_t interpreter_handler_max;
...
#define BREAK_INTERPRETER_LOOP(reason)                                \
  ...
  bool interpreter_grow_stack_internal(address method,
    ...
    g_jpc =  (address)
      (((juint)(address_word)method + Method::base_offset()) + JavaFrame::overflow_frame_flag);
  ...
  static void quick_native_method_entry(BasicType return_type) {
    ...
    jint r1, r2;
    /* NULL cannot initialize va_list on ARM EABI (it is a struct there,
     * while 32-bit x86 hosts accept the implicit char* conversion). A
     * value-initialized temporary works in C++ on both, and ap is never
     * read because num_args is 0. */
  ...
  static address get_static_field_offset() {
    ...
    if (obj == get_cib_marker()) {
      ...
      if (interpreter_call_vm_1((address)task_barrier, T_OBJECT, (jint)(address_word)klass)) {
        return NULL;
      }
...
#define BYTECODE_IMPL_END }
  ...
  // CLDC subclasses jsr/ret out by default, but class files compiled with
  // old javac (< 1.5, target < 1.5) may still contain them (try/finally).
  // ROMized system classes are jsr-free today, yet user MIDlets are not
  // guaranteed to be: without these entries undef_bc() is hit and, because
  // BREAKPOINT is not fatal on Vita, execution continues with a corrupted
  // frame pointer and crashes elsewhere. Implement full JLS semantics.
  BYTECODE_IMPL(jsr)
    PUSH((jint)(address_word)(g_jpc + 3));
    branch(true);
  BYTECODE_IMPL_END

  BYTECODE_IMPL(jsr_w)
    PUSH((jint)(address_word)(g_jpc + 5));
  ...
  BYTECODE_IMPL(tableswitch)
    ...
    address aligned_jpc = (address)((jint)(address_word)(g_jpc + 1 + 3) & ~3);
    // get default target
  ...
  // ret: the operand is the index of the local holding the returnAddress
  // pushed by jsr/jsr_w. Narrow form (1-byte index) and wide form (2-byte
  // index, dispatched through bc_impl_wide with WIDE_OFFSET) both exist.
  // The returnAddress is the absolute bci pushed by jsr, so g_jpc is set
  // directly with no ADVANCE.
  BYTECODE_IMPL_NO_STEP(ret)
    g_jpc = (address)GET_LOCAL(GET_BYTE(0));
  BYTECODE_IMPL_END

  BYTECODE_IMPL_NO_STEP(ret_wide)
    g_jpc = (address)(address_word)GET_LOCAL(GET_SHORT(0));
  BYTECODE_IMPL_END
  ...
  // VITA diagnostic ring: last N dispatched bytecodes with the interpreter
  // stack pointer at dispatch time. When an invariant below trips we dump
  // the window to stderr (midp_stderr.log on device) and stop cleanly.
  // Game-launch wild crash (2026-09-03/04): a bc_impl popped a code pointer
  // into r4 and returned into the dispatch loop, so the crash register dump
  // was far away from the faulting bytecode; this makes the faulting window
  // recoverable from the log.
#define BC_RING_SIZE 64
  static struct { int code; address bcp; address jsp; } bc_ring[BC_RING_SIZE];
  static int bc_ring_idx = 0;
  static unsigned bc_ring_count = 0;

  static void bc_ring_dump(void) {
    int i = bc_ring_idx; /* oldest entry */
    for (int n = 0; n < BC_RING_SIZE; n++) {
      fprintf(stderr, "  [%02d] bcp=%p op=0x%02x jsp=%p\n", n,
              (void*)bc_ring[i].bcp, bc_ring[i].code, (void*)bc_ring[i].jsp);
      i = (i + 1) % BC_RING_SIZE;
    }
  }

  // VITA: one-shot gate for all fatal-report paths. After the first report
  // (full dump + stop) any other thread hitting a fatal condition must exit
  // silently: every lightweight thread runs its own Interpret() loop on the
  // SAME corrupted dispatch table / globals, so without this gate each of
  // them re-reports the full 64-entry breadcrumb dump in a tight loop,
  // flooding midp_stderr.log within seconds (v01.04 "疯狂报错").
  static bool vita_fatal_reported = false;

  // VITA interpreter watchdog heartbeat. The PC=0 crash of v01.05 leaves the
  // process alive: Vita3K only logs "Invalid read at 0x0" from its dynarmic
  // core in an endless loop instead of killing the guest, so the breadcrumb
  // ring still holds the last 64 dispatched bytecodes but nobody can ever
  // print them. A watchdog thread (OS_vita.cpp vita_watchdog_thread) samples
  // this counter; when it freezes while the VM is running it calls
  // vita_interpreter_dump() and exits the process so the evidence lands in
  // midp_stderr.log.
  // Written from every lightweight thread's dispatch loop, only the two
  // adjacent 32-bit halves matter, plain write is sufficient on ARMv7.
  volatile jint  vita_interpreter_heartbeat = 0;
  volatile bool  vita_vm_stopping_cleanly   = false;
  // set by bc_crash_report / undef_bc_stop: the interpreter is reporting and
  // exiting on its own, the watchdog must stand down.
  volatile bool  vita_fatal_reported_flag   = false;

  void vita_interpreter_dump(void) {
    fprintf(stderr,
            "VITA WATCHDOG DUMP: g_jpc=%p g_jsp=%p g_jfp=%p total=%u\n"
            "  last %d dispatched bytecodes (oldest first):\n",
            (void*)g_jpc, (void*)g_jsp, (void*)g_jfp,
            bc_ring_count, BC_RING_SIZE);
    bc_ring_dump();
    fflush(stderr);
  }

#if !defined(PRODUCT) || USE_DEBUG_PRINTING
  static void bc_crash_report(const char* why) {
    vita_fatal_reported_flag = true;
    if (vita_fatal_reported) {
      // Already reported once: exit this thread's interpreter silently.
      Universe::set_stopping();
      BREAK_INTERPRETER_LOOP(JMP_STOPPED_MANUALLY);
    }
    vita_fatal_reported = true;
    char buffer[200];
    buffer[0] = '\0';
    JavaFrame frame(Thread::current());
    Method::Raw m = frame.method();
    if (m.not_null()) {
      m().print_name_to(buffer, sizeof(buffer) - 1);
    }
    fprintf(stderr, "VITA FATAL: %s\n"
            "  g_jpc=%p g_jsp=%p g_jfp=%p total=%u method=%s\n"
            "  last %d dispatched bytecodes (oldest first):\n",
            why, (void*)g_jpc, (void*)g_jsp, (void*)g_jfp,
            bc_ring_count, buffer, BC_RING_SIZE);
    tty->print_cr("VITA FATAL: %s g_jpc=%p g_jsp=%p g_jfp=%p method=%s",
                  why, (void*)g_jpc, (void*)g_jsp, (void*)g_jfp, buffer);
    bc_ring_dump();
    Universe::set_stopping();
    BREAK_INTERPRETER_LOOP(JMP_STOPPED_MANUALLY);
  }
  ...
  static void bc_crash_report(const char* why) {
    ...
    vita_fatal_reported = true;
    fprintf(stderr, "VITA FATAL: %s g_jpc=%p g_jsp=%p g_jfp=%p total=%u\n",
            why, (void*)g_jpc, (void*)g_jsp, (void*)g_jfp, bc_ring_count);
    bc_ring_dump();
    Universe::set_stopping();
    BREAK_INTERPRETER_LOOP(JMP_STOPPED_MANUALLY);
  }
  ...
  // VITA: common clean-stop path for undefined bytecodes. On Vita BREAKPOINT
  // is SWI 1, which Vita3K does not treat as fatal: execution used to resume
  // inside this epilogue and return into the dispatch loop with a corrupted
  // frame anchor, crashing far away from the real cause (game-launch crash,
  // 2026-09-03). Exit the interpreter loop the sanctioned way instead
  // (primordial_to_current_thread uses the same BREAK_INTERPRETER_LOOP path).
  static void undef_bc_stop(int code, address bcp) {
    vita_fatal_reported_flag = true;
    fprintf(stderr,
            "FATAL: Undefined bytecode 0x%02x at bcp %p\n"
            "  stopping VM instead of executing it (old behaviour crashed).\n",
            code, (void*)bcp);
    tty->print_cr("FATAL: Undefined bytecode 0x%02x at bcp %p", code, (void*)bcp);
    Universe::set_stopping();
    BREAK_INTERPRETER_LOOP(JMP_STOPPED_MANUALLY);
  }
#if !defined(PRODUCT) || USE_DEBUG_PRINTING
  // Method::print_name_to is only compiled with debug printing enabled.
  static void undef_bc_report(address bcp, int code) {
    char buffer[200];
    buffer[0] = '\0';
    JavaFrame frame(Thread::current());
    Method::Raw m = frame.method();
    ...
    fprintf(stderr, "FATAL: Undefined bytecode 0x%02x at bcp %p, method %s\n",
            code, (void*)bcp, buffer);
    tty->print_cr("FATAL: Undefined bytecode 0x%02x at bcp %p, method %s",
                  code, (void*)bcp, buffer);
    undef_bc_stop(code, bcp);
  }
#endif

  void undef_bc() {
#if !defined(PRODUCT) || USE_DEBUG_PRINTING
    undef_bc_report(g_jpc, *g_jpc);
#else
    undef_bc_stop((int)*g_jpc, g_jpc);
#endif
  }

  // VITA: called on every sanctioned VM shutdown path (JVM::stop) so the
  // watchdog stands down. Defined here inside extern "C"; declared in
  // JVM.cpp with a local extern declaration.
  void vita_vm_clean_shutdown() {
    vita_vm_stopping_cleanly = true;
  }
...
static void init() {
  ...
  init_dispatch_table();
  // VITA: sample the legal handler window from the initialized table
  // (undef_bc fills every unused slot, so min/max cover exactly the
  // addresses blx may legally reach from this table).
  interpreter_handler_min = interpreter_dispatch_table[0];
  interpreter_handler_max = interpreter_dispatch_table[0];
  for (int i = 1; i < (int)ARRAY_SIZE(interpreter_dispatch_table); i++) {
    func_t h = interpreter_dispatch_table[i];
    if (h < interpreter_handler_min) {
      interpreter_handler_min = h;
    }
    if (h > interpreter_handler_max) {
      interpreter_handler_max = h;
    }
  }
}
#undef MY_GUARANTEE
...
static void Interpret() {
  ...
  if (TraceBytecodes) {
    ...
    for (;;) {
      // VITA invariant tripwires before every dispatch (see bc_crash_report):
      // a corrupted frame anchor (misaligned g_jfp) or a walk-off interpreter
      // stack (g_jsp below the thread limit) means the next dispatch will go
      // wild - report the recent bytecode window and stop cleanly instead.
      if ((((address_word)g_jfp) & 0x3) != 0 || g_jsp < _current_stack_limit) {
        bc_crash_report((((address_word)g_jfp) & 0x3)
                          ? "frame anchor misaligned"
                          : "interpreter sp below stack limit");
      }
      bc_ring[bc_ring_idx].code = *g_jpc;
      bc_ring[bc_ring_idx].bcp  = g_jpc;
      bc_ring[bc_ring_idx].jsp  = g_jsp;
      bc_ring_idx = (bc_ring_idx + 1) % BC_RING_SIZE;
      bc_ring_count++;
      // VITA: watchdog heartbeat (see note at vita_interpreter_heartbeat).
      vita_interpreter_heartbeat++;
      // VITA: validate the handler pointer before blx. A wild entry written
      // into interpreter_dispatch_table by an out-of-bounds store makes blx
      // jump into unmapped memory and kill the process with no chance to
      // report (game-launch crash, v01.03: table entry became 0x125d1784,
      // a Java-heap address; v01.06: entry became 0x8112bbf0, a ROM-text
      // bytecode address that PASSED the old fixed [0x81000000,0x81400000)
      // window because ROM text blocks live inside it, and blx slid through
      // bytecode data into a bogus veneer). The legal window is sampled from
      // the initialized table itself and excludes everything except the real
      // bc_impl_* handlers. Catch corruption here so the breadcrumb window
      // and the faulting slot/index land in the log first, then stop cleanly.
      {
        int code = *g_jpc;
        func_t handler = interpreter_dispatch_table[code];
        if (handler < interpreter_handler_min ||
            handler > interpreter_handler_max) {
          fprintf(stderr,
                  "  slot=%d (0x%02x) handler=%p legal=%p..%p table=%p..%p\n",
                  code, code, (void*)handler,
                  (void*)interpreter_handler_min,
                  (void*)interpreter_handler_max,
                  (void*)&interpreter_dispatch_table[0],
                  (void*)&interpreter_dispatch_table[256 + WIDE_OFFSET]);
          bc_crash_report("dispatch table entry outside handler window");
        }
        handler();
      }
    }
...
JavaFrameDebug * jfp_debug() {
  return (JavaFrameDebug*)( ((int)(address_word)g_jfp) +
                            JavaFrame::stack_bottom_pointer_offset() );
}
