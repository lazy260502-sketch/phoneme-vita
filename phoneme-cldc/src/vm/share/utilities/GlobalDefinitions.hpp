...
// On a 64-bit cross-generator host, OopDesc* is 8 bytes, but BytesPerWord
// remains 4 (target ARM value). The GC bitvector and oop_index must use the
// actual pointer size to stay aligned with OopDesc** pointer arithmetic.
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
const int LogBytesPerOop     = 3;
#else
...
// On 64-bit cross-generator hosts, sizeof(jobject) == 8, but the target
// object layout uses 4-byte oop slots (T_OBJECT_byte_size == BytesPerWord == 4).
// Field offset calculations that must match the target ROM image layout should
// use TargetOopSize instead of sizeof(jobject).
// On 32-bit hosts (including -m32 cross-generator builds), sizeof(jobject) == 4
// which already matches the target, so TargetOopSize == BytesPerWord in all cases.
const int TargetOopSize      = BytesPerWord;

// On 64-bit cross-generator hosts, sizeof(OopDesc) == 8 (one 8-byte _klass
// pointer), but on the 32-bit target sizeof(OopDesc) == 4.  ROM image layout
// calculations must use the target value.
// On 32-bit hosts (including -m32 cross-generator builds), sizeof(OopDesc) == 4
// which already matches the target, so TargetOopDescSize == BytesPerWord in all cases.
const int TargetOopDescSize  = BytesPerWord;
...
// Portability fix: use address_word (uintptr_t) for pointer distance
// so it works on 64-bit build hosts.
#define DISTANCE(low, high)        ((jint) (address_word(high) - address_word(low)))
// Portability fix: use address_word (uintptr_t) instead of jint for
// pointer arithmetic so it works on both 32-bit targets and 64-bit
// build hosts (loopgen/romgen run on x86_64).
...
public:
  ...
  ArrayPointer(address native_addr) :
    // Portability fix: cast through intptr_t to avoid
    // pointer-to-int size warning on 64-bit hosts.
    _heap_array(NULL), _offset((int)(intptr_t) native_addr) {}
...
// Portability fix: overloads for plain int lvalues (host is 64-bit).
// Only needed when intptr_t is a distinct type from int (64-bit host);
// on 32-bit targets where intptr_t == int these would be redefinitions.
#include <cstdint>
#include <climits>
#if INTPTR_MAX != INT_MAX
inline void set_nth_bit(int& x, int n) {
  x |= nth_bit(n);
}
inline void clear_nth_bit(int& x, int n) {
  x &= ~nth_bit(n);
}
#endif
...
#if ROMIZED_PRODUCT
  extern const OopDesc* system_symbols[];
#else
  extern       OopDesc* system_symbols[];
#endif
