...
class ConstantPool: public Oop {
 ...
 private:
  ...
  // Portability fix: on a 64-bit cross-generator host, each constant pool
  // slot must be pointer-sized (sizeof(OopDesc*)) to hold oop entries
  // without truncation. On 32-bit targets sizeof(jint) == sizeof(OopDesc*)
  // so the original behaviour is unchanged.
  int cp_slot_size() const {
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    return (int)sizeof(OopDesc*);
#else
    return (int)sizeof(jint);
#endif
  }
  ...
  int offset_from_index(int index) const {
    ...
    return base_offset() + (index * cp_slot_size());
  }
  ...
  int offset_from_checked_index(int index JVM_TRAPS) const {
    ...
    int result = base_offset() + (index * cp_slot_size());
 ...
 private:
  ...
  void double_parts_at_put(int index, jint msw, jint lsw) {
    ...
    if (MSW_FIRST_FOR_DOUBLE) { 
      ...
      int_field_put(offset_from_index(index) + (int)sizeof(jint), lsw);
    } else { 
      int_field_put(offset_from_index(index), lsw);
      int_field_put(offset_from_index(index) + (int)sizeof(jint), msw);
    }
#if CROSS_GENERATOR && (defined(__x86_64__) || defined(__aarch64__))
    // On 64-bit cross-generator, cp_slot_size() == 8 but long/double
    // must occupy two CP slots (16 bytes) to match the 32-bit target
    // layout where cp_slot_size() == 4 and long/double uses 2 slots.
    // The second slot is unused padding (tag is Invalid placeholder).
    // This keeps generate_fieldmap (i+=2) and variable_oops_do (base++ 
    // per tag) in sync.
    tag_at_put(index + 1, JVM_CONSTANT_Invalid);
#endif
  }
