...
#if defined(VITA)
  {
    /* v01.85: name the class being verified - log11 froze inside
     * verify_class with the site ring as the only witness. v01.86:
     * pass the Symbol length - symbols are not NUL-terminated. */
    Symbol::Raw n = ic->name();
    vita_site_verifier_class(n().base_address(), n().length());
    vita_site_mark();
  }
#endif
...
#if defined(VITA)
  vita_site_mark(); /* v01.84: log10 - class-verify entry marker */
#endif
...
#if defined(VITA)
  vita_site_mark(); /* v01.85: verify_class_internal returned */
#endif
#if ENABLE_PERFORMANCE_COUNTERS
