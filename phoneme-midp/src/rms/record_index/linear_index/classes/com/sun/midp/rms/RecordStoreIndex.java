...
class RecordStoreIndex {
    ...
    int[] getRecordIDs() {
        ...
        while (idx < numRecordIDs) {
            ...
            // VITA FIX (v01.49): a corrupt block header can produce a
            // block size <= 0; advancing by it never reaches dbSize and
            // this loop spins forever (UI freeze). Stop walking instead.
            if (currentSize <= 0) {
                break;
            }
        }
    ...
    protected int getRecordHeader_SearchFromTo(int recordId,
        ...
        while (currentId != recordId) {
            ...
            // VITA FIX (v01.49): corrupt header -> block size <= 0 would
            // spin this search forever. Report "not found" and bail out.
            if (currentSize <= 0) {
                return INVALID_OFFSET;
            }
    ...
    int getFreeBlock(byte[] header) throws IOException {
        ...
        while (currentOffset < recordStore.getSize()) {
            ...
            // VITA FIX (v01.49): corrupt header -> block size <= 0 would
            // spin this scan forever. Same failure contract as a bad
            // header read: throw and let the caller clean up.
            if (currentSize <= 0) {
                throw new IOException();
            }

            // VITA FIX (v01.51): a block that does not fit inside the data
            // area means the bytes walked here are not a valid block header
            // (see the note in RecordStoreImpl.compactRecords). Never hand
            // out an offset inside such a region: report "no free block" so
            // the record is appended at the end of the store instead of
            // being written over unrelated data.
            if (currentSize > recordStore.getSize() - currentOffset) {
                System.out.println("[rms] odd block at " + currentOffset
                    + ": size=" + currentSize + " store="
                    + recordStore.getSize() + ", appending at end");
                return 0;
            }
