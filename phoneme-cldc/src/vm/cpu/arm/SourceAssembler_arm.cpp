...
void SourceAssembler::add_using_gp(Register reg, const char *name, 
  ...
  if (is_rotated_imm(offset)) {
    ...
    eol_comment(buff);
    // Workaround for large GP tables (Vita): original code assumed
    // offset <= 2048 and used two rotated-immediate adds. Use a
    // general 32-bit immediate load instead.
    mov_imm(tmp5, offset, cond);
    add(reg, gp, Assembler::reg(tmp5), cond);
  }
}                  
