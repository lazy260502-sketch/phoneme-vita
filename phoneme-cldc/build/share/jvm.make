...
# Ensure dependencies target is always available
ifeq ($(IsLoopGen), true)
dependencies: $(GEN_DIR)/platform $(GEN_DIR)/jvmconfig.h $(DEPEND_TIMESTAMP)
endif
...
# Unix-style object suffix rule (for host generator stages like loopgen/romgen)
%.o: %.cpp
	$(A)$(CPP) $(CPP_OPT_FLAGS) $(CPP_FLAGS) $(CPP_USE_PCH) -c $<
...
$(JVM_EXE): $(BIN_DIR) $(BUILD_PCH) $(JVMX_LIB) $(JVMTEST_LIB) \
            $(JVM_LIB) $(EXE_OBJS)
	$(A)$(LINK) $(LINK_FLAGS) /out:$@ $(EXE_OBJS) \
            -Wl,--start-group $(JVMX_LIB) $(JVMTEST_LIB) $(JVM_LIB) -Wl,--end-group \
            $(LINK_OPT_FLAGS_$(BUILD)) $(LIBS) $(PCSL_LIBS)
...
CPP_FLAGS               += # -Werror disabled for modern GCC compat
endif
...
# DISABLED for Vita cross build: -static -lm fails on Debian multilib
# (libm.a is not in default static search path), and romgen/loopgen are
# build-time host tools not shipped to end users, so dynamic linking is fine.
...
# Generic pattern rule for compiling .cpp -> .o (Unix-style)
# This overrides GNU make's builtin %.o: %.cpp rule so that CPP_FLAGS
# (which includes -I include dirs) are passed to the compiler.
%.o: %.cpp
	$(A)$(CPP) $(CPP_OPT_FLAGS) $(CPP_FLAGS) $(CPP_USE_PCH) -c $< -o $@
...
# JNI stub for MIDP-side natives is provided in jvmspi.cpp
# (Java_com_sun_cldchi_jvm_JVM_flushJarCaches/monotonicTimeMillis).
# The real implementations live in MIDP (src/vita_stubs.c) and are
# linked into the final eboot.bin via MIDP's native table.
...
$(ROM_GENERATOR): $(CLDC_ZIP) $(Obj_Files) OopMapsSkeleton$(OBJ_SUFFIX) \
	...
	$(A)echo "re-linking ROM generator: $(ROM_GENERATOR)"
	$(A)$(LINK) -o $@ $(Obj_Files) OopMaps$(OBJ_SUFFIX) \
		$(GP_TABLE_OBJ) $(PCSL_LIBS) $(LINK_FLAGS)
	$(A)echo generated `pwd`/$@
else
...
$(DIST_LIB_DIR)/cldcx_classes.zip: $(CLDC_ZIP)
	$(A)echo ... archiving extra classes
	$(A)rm -rf classesx
	$(A)mkdir -p classesx
	$(A)cd classesx; $(JAR) xf $(CLDC_ZIP) com/sun/cldc/io/j2me/socket
